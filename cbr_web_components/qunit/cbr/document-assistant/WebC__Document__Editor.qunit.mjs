...
//         editor = await target_div.append_child(WebC__Document__Editor, {
...
//         assert.equal(editor.constructor.element_name     , 'webc-document-editor'    , 'Has correct element name')
...
//
//         assert.equal(text_editor.value                   , MOCK_CONTENT             , 'Shows correct content')
//         assert.equal(preview.innerHTML                   , '<p>' + MOCK_CONTENT + '</p>', 'Shows correct preview')
//     })
//
...
//     })
//
//     test('auto-saves after delay', async assert => {
//         assert.expect(1)
//         const done = assert.async()
//
//         editor.addEventListener('document-saved', () => {
//             assert.ok(true                                                          , 'Auto-save triggered')
//             done()
//         })
//
//         editor.content = '# Auto-save test'
//         editor.unsaved_changes = true
//         editor.raise_event('content-changed', { content: editor.content })
//
//         // Wait for auto-save timer
//         await editor.wait_for(2100)
//     })
//
...
//
//         window.dispatchEvent(new CustomEvent('changes:accept', {
//             detail: {
//                 changes: true,
...
//
//         window.dispatchEvent(new CustomEvent('version:reset', {
//             detail: {
