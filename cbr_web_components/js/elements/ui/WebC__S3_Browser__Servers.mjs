...
// todo refactor into base class since 90% of this class is the same as WebC__S3_Browser__Server_Requests and WebC__S3_Browser__Servers
export default class WebC__S3_Browser__Servers extends Web_Component {

    static url__api_list_file_contents   = "/api/server/s3-servers/file-contents?file_path="
    static url__api_list_files_metadata  = "/api/server/s3-servers/list-files-metadata?parent_folder="
    static url__api_list_folders         = "/api/server/s3-servers/list-folders?parent_folder="
    ...
    async api_get_files_metadata(parent_folder) {
        const api_path = WebC__S3_Browser__Servers.url__api_list_files_metadata + (parent_folder || '')
        const method   = 'GET'
        const data     = null
    ...
    async api_get_folders(parent_folder) {
        const api_path = WebC__S3_Browser__Servers.url__api_list_folders + (parent_folder || '')
        const method   = 'GET'
        const data     = null
        return await this.api_invoke.invoke_api(api_path, method, data)
    }
    ...
    async on_click__file_link(event) {
        ...
        const api_path = WebC__S3_Browser__Servers.url__api_list_file_contents + file_path
    ...
    async html(folders, files_metadata) {
        ...
        let table_rows    = []
        let table_headers = ['event_id', 'event_type', 'date', 'time', 'level', 'message']
...
WebC__S3_Browser__Servers.define()